...
fetch(history_get_data()[0]).then(data => data.json()).then(data => {
  ...
  append(history_container_wrapper, html_item + '<style>.history_slide{text-align:left;font-size:14px;background:var(--card-bg);display:flex;flex-direction:column;align-items:flex-start;}#history-baidu > .blog-slider__pagination{display:none}.history_slide_time{color:#858585;Font-style:italic;font-weight:lighter;}</style');
