function electric_clock_init () {
  fetch('https://wttr.in/' + returnCitySN["cip"] + '?format="%l+\\+%c+\\+%t+\\+%h"').then(res => res.text()).then(data => {
    ...
    style = `<style>@font-face{font-family:'UnidreamLED';src:url('https://npm.elemecdn.com/eurkon-cdn/hexo/fonts/UnidreamLED.ttf');}#electric_clock{padding:20px}.card-clock{padding:0!important;}.card-background{height:150px;margin:15px;border-radius:8px;box-shadow:inset 3px 3px 18px 0 rgb(50 50 50 / 40%);z-index:2;}.clock-row{font-family:UnidreamLED;font-weight:400;display:flex;justify-content:space-between;flex-wrap:nowrap;white-space:nowrap;}.card-clock-time{flex:1;font-size:2rem;text-align:center;line-height:1.5;}</style>`;
